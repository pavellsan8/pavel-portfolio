    ...
    <div className="min-h-screen bg-custom-color_1 p-5">
      ...
      <div className="p-2 mx-auto">
        <div className="bg-custom-color_3 rounded-lg shadow-lg p-6 mb-6">
          <h1 className="text-3xl font-bold text-custom-color_6 mb-2">Contact Management</h1>
          <p className="text-white mb-4">View and manage all contact submissions</p>
                    ...
                    {sortedData.map((item, index) => (
                      <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        {['name', 'email', 'subject', 'message', 'createdAt'].map(key => {
                          ...
                          return (
                            <td key={`${item.id}-${key}`} className={`px-6 py-4 text-sm ${key === 'email' ? 'text-blue-600' : 'text-gray-500'}`}>
                        ...
                        {Object.keys(data[0])
                          .filter(key => !['id', 'name', 'email', 'subject', 'message', 'createdAt'].includes(key))
                          .map(key => (
                            <td key={`${item.id}-${key}`} className="px-6 py-4 text-sm text-gray-500">
                              {key in item ? formatValue(item[key], key) : '-'}
                            </td>
                          ))
                        }
